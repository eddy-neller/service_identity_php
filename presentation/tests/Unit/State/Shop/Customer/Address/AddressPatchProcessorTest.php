...
use App\Application\Shop\UseCase\Command\Customer\UpdateAddress\UpdateAddressOutput;
...
final class AddressPatchProcessorTest extends TestCase
{
    ...
    public function testProcessWithValidInput(): void
    {
        ...
        $output = new UpdateAddressOutput(new AddressItem($address));
        ...
        $this->commandBus->expects($this->once())
            ->method('dispatch')
            ->willReturnCallback(function ($command) use ($input, $customerId, $output): UpdateAddressOutput {
    ...
    public function testProcessThrowsOnInvalidId(): void
    {
        ...
        $processor->process($input, $this->operation, ['id' => '']);
    }
    ...
    public function testProcessAllowsPartialInput(): void
    {
        ...
        $this->commandBus->expects($this->once())
            ->method('dispatch')
            ->willReturnCallback(function ($command) use ($output): UpdateAddressOutput {
                $this->assertInstanceOf(UpdateAddressCommand::class, $command);

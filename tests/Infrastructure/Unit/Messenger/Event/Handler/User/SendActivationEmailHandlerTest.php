...
use App\Infrastructure\Symfony\Messenger\Event\Handler\User\SendActivationEmailHandler;
...
final class SendActivationEmailHandlerTest extends TestCase
{
    ...
    private SendActivationEmailHandler $handler;
    ...
    protected function setUp(): void
    {
        ...
        $this->handler = new SendActivationEmailHandler(
            $this->repository,
            $this->tokenProvider,
            $this->notifier,
            $this->ledger,
    ...
    public function testItSendsTheActivationEmailWithTheCurrentToken(): void
    {
        $event = $this->event();
        $user = $this->userWithActivationToken('activation-token', $this->now->modify('+1 day'));
        ...
        $this->tokenProvider->expects($this->once())
            ->method('encode')
            ->with('activation-token', $this->callback(static fn (EmailAddress $email): bool => self::EMAIL === $email->toString()))
        ...
        $this->ledger->expects($this->once())
            ...
            ->with($event->eventId(), SendActivationEmailHandler::class);
    ...
    public function testItSkipsAnAlreadyProcessedEvent(): void
    {
        ...
        $this->notifier->expects($this->never())->method('sendActivationEmail');
    ...
    public function testItMarksTheEventAsProcessedWhenTheUserDisappeared(): void
    {
        ...
        $this->logger->expects($this->once())->method('info')->with(
            'Activation email skipped because the user no longer exists.',
    ...
    public function testItMarksTheEventAsProcessedWhenTheTokenIsGone(): void
    {
        ...
        $this->notifier->expects($this->never())->method('sendActivationEmail');
        $this->logger->expects($this->once())->method('info')->with(
            'Activation email skipped because its token is stale.',
    ...
    public function testItMarksTheEventAsProcessedWhenTheTokenExpired(): void
    {
        ...
        $this->repository->expects($this->once())->method('findById')
            ->willReturn($this->userWithActivationToken('stale-token', $this->now->modify('-1 minute')));
        $this->tokenProvider->expects($this->never())->method('encode');
        $this->notifier->expects($this->never())->method('sendActivationEmail');
        $this->logger->expects($this->once())->method('info')->with(
            'Activation email skipped because its token is stale.',
    ...
    private function event(): ActivationEmailRequestedEvent
    {
        return new ActivationEmailRequestedEvent(
    ...
    private function userWithActivationToken(string $token, DateTimeImmutable $expiresAt): User
    {
        $user = $this->user();
        $user->requestActivation($token, $expiresAt, $this->now);

...
namespace App\Domain\SharedKernel\Tests\Unit;
 ...
 *   src   : domain/<Ctx>/src/<SubContext?>/<Category>/<Name>.php
 *   test  : domain/<Ctx>/tests/Unit/<Category>/<SubContext?>/<Name>Test.php
 */
final class DomainTestCoverageTest extends TestCase
{
    ...
    /**
     * Discovers every bounded context under domain/ that exposes a src/ directory.
     *
    ...
    private function domainContexts(): array
    {
        ...
        foreach (scandir($domainDir) ?: [] as $entry) {
            ...
            $src = $domainDir . '/' . $entry . '/src';

            if (is_dir($src)) {
                $contexts[$entry] = [
                    'src' => $src,
                    'tests' => $domainDir . '/' . $entry . '/tests/Unit',
                ];
            }
        }
     ...
     * domain/Shop/src/Ordering/ValueObject/CartId.php
     * → domain/Shop/tests/Unit/ValueObject/Ordering/CartIdTest.php.
     */
